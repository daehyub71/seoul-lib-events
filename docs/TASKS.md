...
- [ ] T4-1. 층별 단면 SVG 레이아웃 설계 (1F~4F, SPEC §2.3 공간 배치)
- [ ] T4-2. `FloorPlanView` — 공간 블록 + 건수 뱃지 + hover/클릭 상태
- [ ] T4-3. 공간 클릭 → EventListPanel 연동, "지도로 돌아가기" 내비게이션
- [ ] T4-4. "자료실 일원"·"기타/미지정" 등 특정 공간에 못 박는 건의 표시 방식 처리 (별도 버킷 UI)

## M5. 달력 뷰

- [ ] T5-1. `lib/recurrence.ts` — 주 반복 일정 요일 전개 (73건 검증)
- [ ] T5-2. 월 그리드 렌더링 + 월 이동 + 연/월 점프 (초기 2025-05)
- [ ] T5-3. 기간 행사 가로 바 레인 배치 (3레인 + "+N건" 오버플로)
- [ ] T5-4. 단일 행사 칩 표시, 날짜 클릭 → EventListPanel
- [ ] T5-5. 밀집 월(2024-11, 2016년대 등) 렌더링 확인, 빈 월 안내 처리
